...
const colors = {
  success: "border-[#DCFCE7] bg-[#F0FDF4]",
...
export const ToastHost = () => {
  ...
  return (
    <div className="fixed right-4 top-4 z-50 flex w-[min(92vw,380px)] flex-col gap-3">
      {toasts.map((toast) => (
        <div key={toast.id} className={`rounded-[16px] border p-4 shadow-soft ${colors[toast.type]}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-bold text-ink">{toast.title}</p>
              {toast.message ? <p className="mt-1 text-sm text-body">{toast.message}</p> : null}
            </div>
